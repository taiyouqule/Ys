...
public class SearchRelativeJsonThread implements Callable<List<EasyItemBean>> {
...
//		System.out.println("maxStr----------->"+maxStr);
	 ...
	 *            查询类型（并查询、或查询）
	 */
	public SearchRelativeJsonThread(String args, String from,
			IEnumSearch.SearchRelationType rType,String[] relativeWord) {
		this.args = args;
		this.from = from;
		this.rType = rType;
	...
	public List<EasyItemBean> call() throws Exception {
		...
		try {
			result = booleanSearch.getZhouJsonResult(args, from, relativeWord);
