...
public class FAQIndexServer {
	...
	private String getLine(String htmlStr) {
		org.jsoup.nodes.Document document = null;
		document = Jsoup.parse(htmlStr);
		Elements meta = document.select(".line");
		return meta.text();
	}
	
	private String getId(String htmlStr) {
		org.jsoup.nodes.Document document = null;
		document = Jsoup.parse(htmlStr);
		Elements meta = document.select(".paperid");
		return meta.text();
	}
...
//			doc.add(c);
			...
			Field lf = new Field("line", line, Field.Store.YES,
					Field.Index.ANALYZED, TermVector.NO);
			doc.add(lf);
			
			
			String paperId = getId(html);
